...
export class ForecastService {
   ...
   */
  public static getDemandForecast(): DemandForecast[] {
    ...
    return services.map(service => {
      ...
      const history = db.demandHistory.filter(h => h.service === service);
      ...
      if (diff > 15) trend = 'UPWARD';
      else if (diff < -15) trend = 'DOWNWARD';

      let forecastLevel: 'HIGH' | 'MEDIUM' | 'LOW' = 'MEDIUM';
      if (avgDaily > 70) forecastLevel = 'HIGH';
      else if (avgDaily < 35) forecastLevel = 'LOW';
   ...
   */
  public static getZoneDemands(): ZoneDemand[] {
    const zones = [
      { name: 'Central Varanasi', baseLevel: 'HIGH' as const, recs: [{ skill: 'Electrician', count: 5 }, { skill: 'Plumber', count: 4 }, { skill: 'AC Technician', count: 3 }] },
      { name: 'North Zone (Orderly Bazaar)', baseLevel: 'HIGH' as const, recs: [{ skill: 'Cleaner', count: 4 }, { skill: 'Painter', count: 2 }] },
      { name: 'South Zone (BHU / Lanka)', baseLevel: 'MEDIUM' as const, recs: [{ skill: 'Plumber', count: 3 }, { skill: 'Carpenter', count: 2 }] },
      { name: 'East Zone (Ghats & Chowk)', baseLevel: 'LOW' as const, recs: [{ skill: 'Electrician', count: 2 }, { skill: 'Gardener', count: 1 }] }
    ];
   ...
   */
  public static getWorkforceRecommendations() {
    const zones = this.getZoneDemands();
    const availableWorkers = Array.from(db.workers.values()).filter(
      w => w.availabilityStatus === 'AVAILABLE' && w.verificationStatus === 'VERIFIED'
    );
