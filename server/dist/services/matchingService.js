...
class MatchingService {
    static findWorkers(criteria) {
        ...
        const matched = workers
            ...
            if (criteria.emergency && w.availabilityStatus !== 'AVAILABLE')
                return false;
            // Availability filter
            if (criteria.onlyAvailable && w.availabilityStatus !== 'AVAILABLE')
                return false;
            // Skill filter
            if (criteria.skillId) {
                const hasSkill = w.skills.some(s => s.skillId.toLowerCase() === criteria.skillId.toLowerCase() || s.name.toLowerCase().includes(criteria.skillId.toLowerCase()));
                if (!hasSkill)
                    return false;
            }
            // Rating filter
            if (criteria.minRating && w.averageRating < criteria.minRating)
                return false;
            // Price filter
            if (criteria.maxPrice && w.startingPrice > criteria.maxPrice)
                return false;
            return true;
        })
            .map(w => {
