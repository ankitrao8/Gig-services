...
class FraudService {
     ...
     */
    static evaluateRating(customerId, workerId, rating, review) {
        // 1. Check if user has submitted multiple ratings in the last 10 minutes
        const customerRatings = Array.from(store_1.db.ratings.values()).filter(r => r.customerId === customerId);
     ...
     */
    static getFlaggedRatings() {
        return Array.from(store_1.db.ratings.values()).filter(r => r.flaggedSuspicious);
    }
}
